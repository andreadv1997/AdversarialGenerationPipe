...
class MultilayerPerceptron():
    ...
    def __init__(self, input_dim):   #metodo costruttore . Definisce il costruttore per la classe che stiamo definendo
        ...
        #    self.mlp = k.models.load_model("mlp.joblib")
        #if (os.path.exists("mlp_SingleScaler.joblib")):
        #    self.mlp = k.models.load_model("mlp_SingleScaler.joblib")
         if (os.path.exists("mlp_NotScaled.joblib")):  #ALTERAZIONE DEL NOME FILE PER CONSENTIRE L'ADDESTRAMENTO OGNI VOLTA. CAMBIARE PER RIPRISTINARE A mlp_NotScaled.joblib
            self.mlp = k.models.load_model("mlp_NotScaled.joblib")
         #if (os.path.exists("mlp_crossEntropy.joblib")):
         #   self.mlp = k.models.load_model("mlp_crossEntropy.joblib")
         else:
            ...
            file_train = base_dir + '/../updatedInput/MOD-training.csv'
            dataset_train,_ = get_data_from_file(file_train)

            file_val = base_dir + '/../updatedInput/MOD-validation.csv'
            ...
            if (dict['accuracy'] >= 0.91 and dict['precision'] >= 0.91 and dict['recall'] >= 0.91 and dict['f1'] >= 0.91):
                    ...
                    # self.mlp.save("mlp.joblib")
                    #self.mlp.save("mlp_SingleScaler.joblib")
                    self.mlp.save("mlp_NotScaled.joblib")
                    #self.mlp.save("mlp_crossEntropy.joblib")
    ...
    def adv_train(self):
        if (os.path.exists("mlp_ADV_TRAIN.joblib")):
            self.mlp = k.models.load_model("mlp_ADV_TRAIN.joblib")
        else:
            base_dir = str(Path().resolve())
            ...
            file_original = base_dir + '/MOD-training.csv'
            dataset_original = get_data_from_file(file_original)
            ...
            pred = self.predict(X_val)
            dict = self.evaluate(pred, Y_val)
            if (dict['accuracy'] >= 0.91 and dict['precision'] >= 0.91 and dict['recall'] >= 0.91 and dict[
                'f1'] >= 0.91):
                ...
                print('Recall: ' + str(dict['recall']))
                print('F1: ' + str(dict['f1']))
                self.mlp.save("mlp_ADV_TRAIN.joblib")
    ...
    def train(self, x, y):
        ...
        batch_size = 32 #divido training set in batch
        validation_split = 0.05 #pezzetto dei record benigni per l'addestramento
