...
from decisionTreeNEW import retrain, decisionTree, compute_metrics as cmDT
...
def mnist_tutorial(train_start=0, train_end=8363, test_start=0,
    ...
    algorithm_under_test="DeepFool_REP_TEST___"

    #state of art test
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'theta': 0.1, 'gamma': 0.7}  # JSMA
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'eps': 0.5}  # FGSM
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'eps': 0.001}  # FGSM
    fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'nb_candidate': 2, 'nb_classes': 2, 'overshoot': 0.1, 'max_iter': 1}  # DeepFool
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'eps': 0.5, 'xi': 0.5, 'num_iterations': 1}  # VirtualADV
    ...
    dict_General_values['params']=fgsm_params
    ...
    if clean_train:
        ...
        adversarial_prediction = preds_adv.eval({x:X_test_adv_mlp}, session=sess)
        adversarial_prediction = np.argmax(adversarial_prediction,axis=1)
        adversarial_prediction = np.where(adversarial_prediction==0,1,0)

        adversarial_prediction = adversarial_prediction.reshape(adversarial_prediction.shape[0],1)
        ...
        adversarial_samples, X_test, Y_test, original_adv_pred = sannity_check_adv(adversarial_s, protocol_test, advScaler, X_test, Y_test, adversarial_prediction)

        #pd.DataFrame(np.concatenate((adversarial_samples, Y_test[:,0].reshape(Y_test.shape[0],1)), axis=1)).to_csv(base_dir+"/adversarials.csv", index=False, header=features)
        #exit()
        ...
        sig_adv, sig_ORIGINAL_Y,index_2_3 = collaborative_filtering(X_FIL, Y_FIL, X_test, Y_test, adversarial_samples, original_adv_pred)
        ...
        #write_Results_On_File(algorithm_under_test + "_DecisionTree", dictionary_X_Test_DT,dictionary_adversarial_samples_DT,dictionary_sig_adv_DT)
        #write_Results_On_File(algorithm_under_test + "_RandomForest", dictionary_X_Test_RF,dictionary_adversarial_samples_RF,dictionary_sig_adv_RF)
        #write_Results_On_File(algorithm_under_test + "_MLP", dictionary_X_Test_MLP,dictionary_adversarial_samples_MLP,dictionary_sig_adv_MLP)
        ...
        dt = retrain(adversarial_samples, Y_test) #addestramento solo con i campioni non aggiornati/filtrati



        fgsm_params_rep_test = {'clip_min': 0.0, 'clip_max': 1.8, 'eps': 0.01, 'xi': 0.01, 'num_iterations': 1}  # VirtualADV
        fgsm_rep_test = VirtualAdversarialMethod(model, sess=sess)
        adv_x_rep_test = fgsm_rep_test.generate(x, **fgsm_params_rep_test)
        adv_model_pred = model.get_probs(adv_x_rep_test)

        adv_model_pred = adv_model_pred.eval({x:X_test_adv_mlp}, session = sess)
        adv_model_pred = np.argmax(adv_model_pred,axis=1)
        adv_model_pred = np.where(adv_model_pred==0,1,0)
        adv_model_pred = adv_model_pred.reshape((adv_model_pred.shape[0],1))

        adv_x_rep_test = adv_x_rep_test.eval({x:X_test_adv_mlp}, session=sess)
        adversarial_samples_test, _, Y_rep_test, adv_p = sannity_check_adv(adv_x_rep_test, protocol_test, advScaler, X_test_adv_mlp, original_Y_Test, adv_model_pred)
        adversarial_samples_test, label_comp,_ = collaborative_filtering(X_FIL, Y_FIL, X_test, Y_rep_test,adversarial_samples_test, adv_p)
        ...
        #GENERAZIONE DEGLI ESEMPI ASSOCIATI AI TRE DIVERSI ALGORITMI PER ADDESTRARE LA RETE
        ...
        adversarial_samples_alg2, _, Y_alg2, adv_p_alg2 = sannity_check_adv(adv_x_alg2, protocol_test, advScaler,X_test_adv_mlp, original_Y_Test,adv_model_pred_alg2)
        adversarial_samples_alg2, label_comp_alg2, index_alg2 = collaborative_filtering(X_FIL, Y_FIL, X_test, Y_alg2,adversarial_samples_alg2, adv_p_alg2)

        fgsm_params_alg3  = {'clip_min': 0.0, 'clip_max': 1.8, 'nb_candidate': 2, 'nb_classes': 2, 'overshoot': 0.1, 'max_iter': 1}  # DeepFool
        ...
        adversarial_samples_alg3, _, Y_alg3, adv_p_alg3 = sannity_check_adv(adv_x_alg3, protocol_test, advScaler,X_test_adv_mlp, original_Y_Test,adv_model_pred_alg3)
        adversarial_samples_alg3, label_comp_alg3, index_alg3 = collaborative_filtering(X_FIL, Y_FIL, X_test, Y_alg3,adversarial_samples_alg3, adv_p_alg3)
        ...
        dt = retrain(train_adv_samples, train_adv_labels)
        #dt=  retrain(sig_adv[index_2_3], sig_ORIGINAL_Y[index_2_3])

        prediction_rep = dt.predict(adversarial_samples_test)
        dict_rep = cmDT(Y_rep_test,prediction_rep)
        print('Accuracy of dt: ' + str(dict_rep['accuracy']))
        print('Precision of dt: ' + str(dict_rep['precision']))
        print('Recall of dt: ' + str(dict_rep['recall']))
        print('F1 of dt: ' + str(dict_rep['f1']))
...
def collaborative_filtering(x_fil, y_fil, X_test, Y_test, adversarial_samples, predictions):
    ...
    computed_labels = np.zeros(adversarial_samples.shape[0])
    ...
    for index in range(adversarial_samples.shape[0]):
        ...
        if (score < 0.2):
            ...
            label_rec[index] = Y_test[index,0] #considerao come label quella originaria
            if(label_rec[index]==1):
                doubt_B += 1
                computed_BENIGN+=1
            else:
                doubt_A+=1
    ...
    #in questa nuova versione bisogna far riferimento alla classificazione su ipad
    index_2_3=[]
    case_1 = 0
    case_1a = 0
    case_1b = 0
    ...
    for ind in range(label_count.shape[0]):
        ...
        if (label_count[ind] != 0): #case 1 o 3
            if(predictions[ind,0]==label_rec[ind]):
                case_1+=1
                if(Y_test[ind,0]==1):
                    case_1a+=1
                else:
    ...
    print("% case 1: ", case_1 / label_count.shape[0], "--- ", str(case_1))
    print("% case 1a: ", case_1a / label_count.shape[0], "--- ", str(case_1a))
    print("% case 1b: ", case_1b / label_count.shape[0], "--- ", str(case_1b))
    print("% case 2: ", case_2 / label_count.shape[0], "--- ", str(case_2))
    print("% case 2a: ", case_2a / label_count.shape[0], "--- ", str(case_2a))
    print("% case 2b: ", case_2b / label_count.shape[0], "--- ", str(case_2b))
    print("% case 3: ", case_3 / label_count.shape[0], "--- ", str(case_3))
    print("% case 3a: ", case_3a / label_count.shape[0], "--- ", str(case_3a))
    print("% case 3b: ", case_3b / label_count.shape[0], "--- ", str(case_3b))
    print("% case 4: ", case_4 / label_count.shape[0], "--- ", str(case_4))
    print("% case 4a: ", case_4a / label_count.shape[0], "--- ", str(case_4a))
    print("% case 4b: ", case_4b / label_count.shape[0], "--- ", str(case_4b))

    dict_General_values['case_1'] = case_1
    dict_General_values['case_1a'] = case_1a
    dict_General_values['case_1b'] = case_1b
    dict_General_values['case_2'] = case_2
    dict_General_values['case_2a'] = case_2a
    dict_General_values['case_2b'] = case_2b
    dict_General_values['case_3'] = case_3
    dict_General_values['case_3a'] = case_3a
    dict_General_values['case_3b'] = case_3b
    dict_General_values['case_4'] = case_4
    dict_General_values['case_4a'] = case_4a
    dict_General_values['case_4b'] = case_4b
    dict_General_values['originally_BENIGN'] = originally_BENIGN
    dict_General_values['originally_ATTACK'] = originally_ATTACK
    dict_General_values['computed_BENIGN'] = computed_BENIGN
    dict_General_values['computed_ATTACK'] = computed_ATTACK
    dict_General_values['doubt_B'] = doubt_B
    dict_General_values['doubt_A'] = doubt_A

    return adversarial_samples, np.asarray(label_rec).reshape(adversarial_samples.shape[0],1), index_2_3
...
def sannity_check_adv(adversarial_samples, protocol_test, scaler, X_test, Y_test, original_adversarial_prediction):
    ...
    while i < shape:
         if (np.isinf(not_scaled_samples[i]).any() or np.isnan(not_scaled_samples[i]).any()):
              ...
              X_test = np.delete(X_test, obj=i, axis=0)
              Y_test = np.delete(Y_test, obj=i, axis=0)
              original_adversarial_prediction = np.delete(original_adversarial_prediction, obj=i, axis=0)
    ...
    return not_scaled_samples, X_test, Y_test, original_adversarial_prediction
...
def write_NON_REP_RESULTS_On_File(fileName, dict_ORIGINAL, dict_WITH_ORIGINAL_LABEL, dict_WITH_COMPUTED_LABEL):
    ...
    outputDirTXT = base_dir + '/outputTXT_RESULTS_ON_FILTERED_DATA/newVersionRep'
...
def write_Results_On_File(fileName, dict_X_Test, dict_Adv_Not_Filtered, dict_Adv_Filtered):

    outputDirTXT = base_dir + '/outputTXT_NotScaled/newRepr'
    outputDirCSV = base_dir + '/outputCSV_NotScaled/newRepr'
    ...
    with open(txtFile, "w") as file1, open(csvFile,"w") as file2 :
        ...
        file1.write('Metric\t\tOriginal Test Set (X_test)\t\tNot Filtered Adversarial Samples(adv_x)\t\t\tFiltered Adversarial Samples(sig_adv_sample)\n')
        file1.write('Accuracy\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['accuracy']), str(dict_Adv_Not_Filtered['accuracy']), str(dict_Adv_Filtered['accuracy'])))
        file1.write('Precision\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['precision']), str(dict_Adv_Not_Filtered['precision']), str(dict_Adv_Filtered['precision'])))
        file1.write('Recall\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['recall']), str(dict_Adv_Not_Filtered['recall']), str(dict_Adv_Filtered['recall'])))
        file1.write('F1\t\t%s\t\t%s\t\t%s' % (str(dict_X_Test['f1']), str(dict_Adv_Not_Filtered['f1']), str(dict_Adv_Filtered['f1'])))

        file1.write('\n##################\n')

        file1.write('MLP Accuray\t\t%s\n' % (str(dict_General_values['MLPaccuracy'])))
        file1.write('Total samples\t\t%s\n' % (str(dict_General_values['total'])))
        file1.write('ORIGINALLY BENIGN samples\t\t%s\t\t%s %%\n' % (str(dict_General_values['originally_BENIGN']),str(float(dict_General_values['originally_BENIGN'])/float(dict_General_values['total'])) ))
        file1.write('ORIGINALLY ATTACK samples\t\t%s\t\t%s %%\n' % (str(dict_General_values['originally_ATTACK']),str(float(dict_General_values['originally_ATTACK'])/float(dict_General_values['total'])) ))
        file1.write('COMPUTED BENIGN samples\t\t%s (%s Doubt)\t\t%s %%\n' % (str(dict_General_values['computed_BENIGN']),str(dict_General_values['doubt_B']), str(float(dict_General_values['computed_BENIGN']) / float(dict_General_values['total']))))
        file1.write('COMPUTED ATTACK samples\t\t%s (%s Doubt)\t\t%s %%\n' % (str(dict_General_values['computed_ATTACK']),str(dict_General_values['doubt_A']), str(float(dict_General_values['computed_ATTACK']) / float(dict_General_values['total']))))

        file1.write('Case 1\t\t%s\t\t%s %%\ta)%s    b)%s\n' % (str(dict_General_values['case_1']), str(float(dict_General_values['case_1'])/float(dict_General_values['total'])),str(dict_General_values['case_1a']),str(dict_General_values['case_1b'])))
        file1.write('Case 2\t\t%s\t\t%s %%\ta)%s    b)%s\n'%(str(dict_General_values['case_2']), str(float(dict_General_values['case_2'])/float(dict_General_values['total'])),str(dict_General_values['case_2a']),str(dict_General_values['case_2b'])))
        file1.write('Case 3\t\t%s\t\t%s %%\ta)%s    b)%s\n'%(str(dict_General_values['case_3']), str(float(dict_General_values['case_3'])/float(dict_General_values['total'])),str(dict_General_values['case_3a']),str(dict_General_values['case_3b'])))
        file1.write('Case 4\t\t%s\t\t%s %%\ta)%s    b)%s\n' % (str(dict_General_values['case_4']), str(float(dict_General_values['case_4'])/float(dict_General_values['total'])),str(dict_General_values['case_4a']),str(dict_General_values['case_4b'])))
        file1.write('Params\t\t%s\n' % (str(dict_General_values['params'])))

        file1.write('\n##################\n')
        file1.write('Metric\t\tOriginal Test Set (X_test)\t\tNot Filtered Adversarial Samples(adv_x)\t\t\tFiltered Adversarial Samples(sig_adv_sample)\n')
        file1.write('True Positive\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['true_pos']), str(dict_Adv_Not_Filtered['true_pos']), str(dict_Adv_Filtered['true_pos'])))
        file1.write('True Negative\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['true_neg']), str(dict_Adv_Not_Filtered['true_neg']), str(dict_Adv_Filtered['true_neg'])))
        file1.write('False Positive\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['false_pos']), str(dict_Adv_Not_Filtered['false_pos']), str(dict_Adv_Filtered['false_pos'])))
        file1.write('False Negative\t\t%s\t\t%s\t\t%s\n' % (str(dict_X_Test['false_neg']), str(dict_Adv_Not_Filtered['false_neg']), str(dict_Adv_Filtered['false_neg'])))
