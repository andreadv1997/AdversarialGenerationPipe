...
from decisionTreeNEW import retrain, decisionTree, compute_metrics as cmDT, adversarialTraining as advTrainDT
from randomForest import randomForest, compute_metrics as cmRF, adversarialTraining as advTrainRF
import pandas as pd
...
def mnist_tutorial(train_start=0, train_end=8363, test_start=0,
    ...
    algorithm_under_test="VAM_REP_TEST_WITH_FILTERED__"
    adv_file_name ="VAM_adv_01_05_10.csv"

    ##NB RIESEGUIRE TRANING ADV DEEPFOOL PER RIGENERARE FILE ALTERATO

    #state of art test
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.1, 'theta': 0.7, 'gamma': 0.3}  # JSMA
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.1, 'eps': 0.01}  # FGSM
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.8, 'eps': 0.001}  # FGSM
    #fgsm_params = {'clip_min': 0.0, 'clip_max': 1.1, 'nb_candidate': 2, 'nb_classes': 2, 'overshoot': 0.7, 'max_iter': 10}  # DeepFool
    fgsm_params = {'clip_min': 0.0, 'clip_max': 1.1, 'eps': 0.1, 'xi': 0.5, 'num_iterations': 10}  # VirtualADV
    ...
    if clean_train:
        ...
        adversarial_samples, X_test, Y_test = sannity_check_adv(adversarial_s, protocol_test, advScaler, X_test, Y_test)
        ...
        sig_adv, original_sig_samples, sig_ORIGINAL_Y, non_rep = collaborative_filtering(X_FIL, Y_FIL, X_test, Y_test, adversarial_samples)

        #original_sig_samples = new_scaler.transform(notScaled_original_sig_samples)
        #sig_adv = new_scaler.transform(notScaled_sig_adv)


        pd.DataFrame(np.concatenate((sig_adv,sig_ORIGINAL_Y[:,0].reshape(sig_ORIGINAL_Y.shape[0],1)),axis=1)).to_csv(base_dir+"/adversarial_samples/"+adv_file_name, header=features, index=False)
        #exit()
        ...
        adversarial_samples_test, _, Y_rep_test, non_rep = collaborative_filtering(X_FIL, Y_FIL, np.concatenate((protocol_test.reshape(protocol_test.shape[0], 1),X_test_adv_mlp),axis=1), Y_rep_test,adversarial_samples_test)
...
def collaborative_filtering(x_fil, y_fil, X_test, Y_test, adversarial_samples):
    ...
    label_computer_nonRep = np.asarray(label_computer_nonRep)
    label_computer_nonRep = label_computer_nonRep.reshape(nonRep_ORIGINAL_Y.shape[0],1)
    #print("************VALORI CALCOLATI SUI SAMPLE NON RAPPRESENTATIVI****************")
    #computed_metrics_for_NonRep(nonRep_adv, original_nonRep_samples, nonRep_ORIGINAL_Y, label_computer_nonRep)
    #print("***************************************************************************")


    filter_data = np.concatenate((x_fil,y_fil[:,0].reshape(x_fil.shape[0],1)),axis=1)
    nonRep_data = np.concatenate((nonRep_adv,nonRep_ORIGINAL_Y[:,0].reshape(nonRep_adv.shape[0],1)),axis=1)

    #print(filter_data.shape)
    #print(nonRep_data.shape)
    #save_on_file(filter_data, nonRep_data)



    return sig_adv, original_sig_samples, sig_ORIGINAL_Y, nonRep_adv
...
def sannity_check_adv(adversarial_samples, protocol_test, scaler, X_test, Y_test):
...
def write_NON_REP_RESULTS_On_File(fileName, dict_ORIGINAL, dict_WITH_ORIGINAL_LABEL, dict_WITH_COMPUTED_LABEL):
    ...
    outputDirTXT = base_dir + '/outputTXT_RESULTS_ON_FILTERED_DATA'
...
def write_Results_On_File(fileName, dict_X_Test, dict_Adv_Not_Filtered, dict_Original_Filtered, dict_Adv_Filtered):
    ...
    outputDirTXT = base_dir + '/outputTXT_NotScaled'
    outputDirCSV = base_dir + '/outputCSV_NotScaled'
