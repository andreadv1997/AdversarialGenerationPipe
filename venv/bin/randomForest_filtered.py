...
def adversarialTraining():
    if (os.path.exists("randomForest_filtered_ADV_TRAIN.joblib")):
        return load('randomForest_filtered_ADV_TRAIN.joblib')
    else:
...
def retrain(samples_to_add, Y_to_add):
    ...
    if (dict['accuracy'] >= 0.99 and dict['precision'] >= 0.99 and dict['recall'] >= 0.99 and dict['f1'] >= 0.99):
        ...
        dump(clf, 'randomForest_filtered_ADV_TRAIN.joblib')
...
def randomForest(gridSearch = False):
    ...
    if gridSearch:
        ...
        X_train = np.concatenate((X_train, X_val), axis=0)
        Y_train = np.concatenate((Y_train, Y_val), axis=0)
        clf = GridSearchCV(ensemble.RandomForestClassifier(n_jobs=-1, n_estimators=50, max_depth=19, min_samples_leaf=5, random_state=5), param_grid=parameters,refit=True, n_jobs=-1) #5-fold
        clf.fit(X_train, Y_train)

    else:

        clf = load('randomForest_filtered_NotScaled.joblib')
    ...
    if (dict['accuracy'] >= 0.99 and dict['precision'] >= 0.99 and dict['recall'] >= 0.99 and dict['f1'] >= 0.99):
        ...
        dump(clf, 'randomForest_filtered_NotScaled.joblib')

    else:
        print("Values DO NOT satisfy constraints")
        print('Accuracy of decision tree: ' + str(acc))
        print('Precision of decision tree: ' + str(prec))
        print('Recall of decision tree: ' + str(rec))
        print('F1 of decision tree: ' + str(f1_t))
        return None
...
if __name__ == '__main__':
    ...
    #dt,_,_,_,_,scaler = randomForest()
    from randomForest import randomForest as oldRF
    dt,_,_,_,_,_ = oldRF()
